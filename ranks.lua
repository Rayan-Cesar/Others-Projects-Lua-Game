local displayoutput = {"Fusion Kill","Resets","Level","Magic Level","Fist Fighting","Club Fighting","Sword Fighting","Axe Fighting","Distance Fighting","Shielding"}

local itemtype = {11192,2229,5903,5896,8929,8932,7388,8849,8906,2580}
...
if display == 1 then
            local player = db.getResult("SELECT `player_storage`.`key`, `name`, `player_storage`.`value` FROM `player_storage`, `players` where `players`.`id` = `player_storage`.`player_id` and `players`.`group_id` < 4 and `player_storage`.`key` = "..STORAGE_FUSION_KILL.." ORDER BY `value` DESC LIMIT 50;")
